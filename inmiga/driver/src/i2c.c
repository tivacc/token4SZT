 ...
 **
 ** I2C driver define.
 ** @link I2C Group Some description @endlink
 **
...
#define I2C_INSTANCE_COUNT  6

#define IS_VALID_CH(x)              (   I2CCH0 == (x) ||\
                                        I2CCH1 == (x) ||\
                                        I2CCH2 == (x) ||\
                                        I2CCH4 == (x) ||\
                                        I2CCH5 == (x) ||\
...
#define IS_VALID_IRQ(x)             (   I2cTxIrq ==(x) || \
                                        I2cRxIrq ==(x) || \
                                        I2cTxRxIrq ==(x) || \
                                        I2cStopDetectIrq ==(x) )

#define IS_VALID_STAT(x)            (   I2cOverrunError == (x)||\
                                        I2cRxFull == (x)||\
                                        I2cTxEmpty == (x)||\
...
#define IS_VALID_IRQTYPE(x)         (   I2cIrqRxEn == (x)||\
                                        I2cIrqTxEn == (x) )

#define IS_VALID_I2CMODE(x)         (   I2cMaster == (x)||\
                                        I2cSlave == (x) )

#define IS_VALID_ACK(x)             (   I2cAck == (x)||\
                                        I2cNAck == (x) )
...
stc_i2c_instance_data_t m_astcI2cDataLut[I2C_INSTANCE_COUNT] =
{
    {
        I2CCH0,         // Ch.0
        M0P_MSC0_I2C,   // pstcInstance
        {NULL,NULL,NULL,NULL},         // stcInternData (not initialized yet)
    },
    {
        I2CCH1,         // Ch.1
        M0P_MSC1_I2C,   // pstcInstance
        {NULL,NULL,NULL,NULL},              // stcInternData (not initialized yet)
    },
    {
        I2CCH2,         // Ch.2
        M0P_MSC2_I2C,   // pstcInstance
        {NULL,NULL,NULL,NULL},              // stcInternData (not initialized yet)
    },
    {
        I2CCH4,         // Ch.4
 ...
 **
 ** \param u8Idx Channel of the I2C
 **
 ...
 ** \param enIrqType  I2cIrqRxEn: RX IRQ, I2cIrqTxEn: TX IRQ
 **
 ******************************************************************************/
static void I2cInitNvic(uint8_t u8Channel, en_i2c_irq_type_t enIrqType)
{
    ...
    switch (u8Channel)
    {
        ...
        case I2CCH1:
        case I2CCH2:
            enIrq =(IRQn_Type) (MSC0RX_IRQn + u8Channel*2);
            break;
        case I2CCH4:
        case I2CCH5:
        case I2CCH6:
            enIrq =(IRQn_Type) (MSC4RX_IRQn + (u8Channel - 4)*2);
            break;
        default:
            return;
    }
...
/**
 ******************************************************************************
 ** \brief Device dependent de-initialization of transmit interrupts
 **        according CMSIS
 **
 ** \param u8Channel   I2C channel
 ** \param enIrqType  I2cIrqRxEn: RX IRQ, I2cIrqTxEn: TX IRQ
...
static void I2cDeInitNvic(uint8_t u8Channel)
{
    ...
    switch (u8Channel)
    {
        ...
        case I2CCH1:
        case I2CCH2:
            enIrq =(IRQn_Type) (MSC0RX_IRQn + u8Channel);
            break;
        case I2CCH4:
        case I2CCH5:
        case I2CCH6:
            enIrq =(IRQn_Type) (MSC4RX_IRQn + u8Channel - 4);
            break;
        default:
            return;
    }
 ...
 **
 ** \param u8Channel    Channel of I2C
 **
 ** This function is called on Receive Interrupt set by the MSC.
 **
 ******************************************************************************/
 ...
 ** \param u8Channel    Channel of I2C
 **
 ** This function is called on transfer Interrupt set by the MSC.
 **
 ******************************************************************************/
...
void I2c_Stat_IrqHander( uint8_t u8Channel)
{
    ...
    if(1u == pstcI2c->BC_f.INTF)
    {
        ...
        // pstcI2c->BS_f.INTF = 0; /* It will be cleared in callback function */
    }
...
/**
 ******************************************************************************
 ** \brief Initialization of a MSC module to activate as I2C.
 **
 ** Set registers to active MSC as I2C.
 **
 ** \param [in]   u8Channel       Channel of I2C
...
en_result_t I2c_Init(uint8_t u8Channel, stc_i2c_config_t* pstcConfig)
{
    ...
    /* Set wait bit  */
    stcBC.WTSEL = pstcConfig->bWaitSelection;
...
/**
 ******************************************************************************
 ** \brief Deinitialisation of a MSC module activating as I2C.
 **
...
en_result_t I2c_SetBaudRate(uint8_t u8Channel, uint32_t u32BaudRate)
{
    ...
    ASSERT(IS_VALID_CH(u8Channel));
    ASSERT(u32BaudRate<=400000u);

    pstcI2cInternData = I2cGetInternDataPtr(u8Channel);
    if ((NULL == pstcI2cInternData) || (u32BaudRate > 400000u))
    {
        return ErrorInvalidParameter;
    }
...
en_result_t I2c_GenerateStop(uint8_t u8Channel)
{
    ...
    while(u32TimeOut > 0)
    {
        u32TimeOut--;
        pstcI2c->BC &= ~0xc1u;
        if((0u == pstcI2c->BC_f.MSS) && (0 == pstcI2c->BC_f.OFITS))
        {
            return (Ok);
        }
    }
 ...
 **
 ******************************************************************************/
uint8_t I2c_ReceiveData(uint8_t u8Channel)
{
    stc_i2c_instance_data_t* pstcI2cInternData = NULL;
    volatile stc_mscn_i2c_t *pstcI2c = NULL;
