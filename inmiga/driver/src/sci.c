...
{
    if((1u == M0P_SCI->STATUS_f.TX_FIN)||(1u == M0P_SCI->STATUS_f.TX_ERR))
    { 
        //M0P_SCI->STATUS_f.TX_FIN = 0;
        if(NULL != stcSciIrqCb.pfnTxIrqCb)
        {
            stcSciIrqCb.pfnTxIrqCb();
        }
    }
    if((1u == M0P_SCI->STATUS_f.RX_FIN)||((1u == M0P_SCI->STATUS_f.RX_ERR)))
    {
    ...
    {
        M0P_SCI->STATUS_f.ETU_CNT_FIN = 0;
    ...
    {
        M0P_SCI->WAKEUP_INT_f.WAKEUP_INT = 0;
...
{    
    M0P_SCI->CON_f.TX_FIFO_CLR = 1u;  //0u  20180523  modify
...
{    
    M0P_SCI->CON_f.RX_FIFO_CLR = 1u;  ////0u  20180523  modify
