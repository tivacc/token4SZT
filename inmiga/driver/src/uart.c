...
#define IS_VALID_CH(x)          (UARTCH0 == (x) ||\
                                 ...
                                 UARTCH5 == (x) ||\
                                 UARTCH6 == (x) )
#define IS_VALID_DSCH(x)        (UARTCH4 == (x) ||\
                                 UARTCH5 == (x) ||\
                                 UARTCH6 == (x) )
#define IS_VALID_IRQSEL(x)      (UartCtsIrq >= (x))
#define IS_VALID_FUNC(x)        ((UartTx == (x)) || (UartRx == (x)))
#define IS_VALID_STATUS(x)      (UartCts >= (x))

/**
 *****************************************************************************
 ** \brief UART instance data type
 *****************************************************************************/
...
/// Look-up table for all enabled MSC instances and their internal data
static stc_uart_instance_data_t m_astcUartInstanceDataLut[] =
{
    {
        UARTCH0,
        M0P_MSC0_UART,       /* pstcInstance */
        {NULL,NULL,NULL,NULL,NULL},
    },
    {
        UARTCH1,
        M0P_MSC1_UART,       /* pstcInstance */
        {NULL,NULL,NULL,NULL,NULL},
    },
    {
        UARTCH2,
        M0P_MSC2_UART,       /* pstcInstance */
        {NULL,NULL,NULL,NULL,NULL},
    },
    {
        UARTCH4,
...
static void UartInitNvic(uint8_t u8Idx, boolean_t bRxTx)
{
    ...
    if(UARTCH2 >= u8Idx)
    {
        if(FALSE == bRxTx)
        {
            enIrqIndex = (IRQn_Type)(MSC0RX_IRQn + (2*u8Idx));
        }
        else
        {
            enIrqIndex = (IRQn_Type)(MSC0TX_IRQn + (2*u8Idx));
        }
    }
    ...
    switch(u8Idx)
    {
        case UARTCH0:
        {
            if(FALSE == bRxTx)
            {
                NVIC_SetPriority(enIrqIndex, DDL_IRQ_LEVEL_MSC0RX);
            }
            else
            {
                NVIC_SetPriority(enIrqIndex, DDL_IRQ_LEVEL_MSC0TX);
            }
            break;
        }
        case UARTCH1:
        {
            if(FALSE == bRxTx)
            {
                NVIC_SetPriority(enIrqIndex, DDL_IRQ_LEVEL_MSC1RX);
            }
            else
            {
                NVIC_SetPriority(enIrqIndex, DDL_IRQ_LEVEL_MSC1TX);
            }
            break;
        }
        case UARTCH2:
        {
            if(FALSE == bRxTx)
            {
                NVIC_SetPriority(enIrqIndex, DDL_IRQ_LEVEL_MSC2RX);
            }
            else
            {
                NVIC_SetPriority(enIrqIndex, DDL_IRQ_LEVEL_MSC2TX);
            }
            break;
        }
        case UARTCH4:
 ...
 ** \arg UartCtsIrq        UART CTS interrupt
 **
 ...
 **
 ** \param [in]   u8Idx           UART channel
 ** \param [in]   pstcConfig      MSC UART configuration
 **
 ** \retval Ok                        Process successfully done.
 ** \retval ErrorInvalidParameter     If one of following conditions are met:
 **             - stc_uart_config_t == NULL
 **
 **
 ******************************************************************************/
                      ...
                      stc_uart_config_t* pstcConfig)
{
    ...
    ASSERT(IS_VALID_EN(pstcConfig->bCtsWu));
    ...
    {
        case UartNormal:
            stcMR.MODE = 0u;   /* Normal mode */
            break;
        case UartMulti:
            stcMR.MODE = 1u;    /* Multi-processor mode */
            break;
        default:
            return (ErrorInvalidParameter);
    }
    ...
    {
        case UartParityNone:
            stcECR.PS = FALSE; /* Parity type selection (0-even/1-odd) */
            stcECR.PE = FALSE; /* Parity disable */
            break;
        case UartParityEven:
            stcECR.PS = FALSE; /* Parity type selection (0-even/1-odd) */
            stcECR.PE = TRUE;  /* Parity Enable */
            break;
        case UartParityOdd:
            stcECR.PS = TRUE;  /* Parity type selection (0-even/1-odd) */
            stcECR.PE = TRUE;  /* Parity Enable */
            break;
        default:
            return (ErrorInvalidParameter);
...
en_result_t Uart_DeInit(uint8_t u8Idx, boolean_t bTouchNvic)
{
    ...
    /* Reset MSC receive and transmit bit to default */
    ...
    /* Clear MSC by setting the Software Reset bit */
    pstcData->pstcInstance->CR_f.PCL = TRUE;
 ...
 ** \arg   UartCts           UART CTS level change
 ** \arg   UartSin           UART one frame received
 **
 ...
 **                               - No CTS level change [enStatus = UartCts]
 **                               - Not receive one frame [enStatus = UartSin]
 ...
 **                               - CTS level change [enStatus = UartCts]
 **                               - Receive one frame [enStatus = UartSin]
 **
 ******************************************************************************/
...
uint16_t Uart_ReceiveData(uint8_t u8Idx)
{
    ...
    return (pstcData->pstcInstance->RXDR);
}
