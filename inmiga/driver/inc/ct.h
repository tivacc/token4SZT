 ...
 **
 ** 
 ** <b>How to use PWM timer function of CT module?</b>
 **
 ** Ct_ConfigIOMode() must be used to configure CT I/O mode to I/O mode 0 first.
 **
 ** Ct_Pwm_Init() is used to initialize PWM timer source clock, output polarity,
 ** operation mode and so on. Following operation mode can set:
 ** - Continuous mode
 ** - One-shot mode
 **
 ** The interrupts can be configured first in Ct_Pwm_Init(), after initialization, 
 ** a PWM interrupt can be enabled by the function Ct_Pwm_EnableIrq() and a PWM 
 ** interrupt can be disabled by the function Ct_Pwm_DisableIrq()
 **
 ** With Ct_Pwm_WriteCycleVal() the PWM cycle counter is set to the value
 ** given in the parameter Ct_Pwm_WriteCycleVal#u16Cycle.
 **
 ** With Ct_Pwm_WriteDutyVal() the PWM duty counter is set to the value
 ** given in the parameter Ct_Pwm_WriteDutyVal#u16Duty.
 **
 ** Notes that PWM can be only set to 16-bit mode, so above two parameters
 ** should be 16-bit.
 **
 ** Ct_Pwm_EnableCount() is used to enabe PWM counter.
 **
 ** After above setting is done, calling Ct_Pwm_EnableSwTrig() will start
 ** PWM timer.
 **
 ** With Ct_Pwm_ReadCurCnt() the current PWM count can be read when PWM timer
 ** is running.
 **
 ...
 **
 ** With polling mode, user can use Ct_Pwm_GetIrqFlag() to check if the
 ** interrupt occurs, and clear the interrupt flag by Ct_Pwm_ClrIrqFlag().
 **
 ** When stopping the PWM timer, use Ct_Pwm_DisableCount() to disable PWM
 ** counter.
 **
 ** <b>How to use PPG timer function of CT module?</b>
 **
 ** Ct_ConfigIOMode() must be used to configure Ct I/O mode to I/O mode 0 first.
 **
 ** Ct_Ppg_Init() is used to initialize PPG timer source clock, output polarity,
 ** operation mode and so on. Following operation mode can set:
 ** - Continuous mode
 ** - One-shot mode
 **
 ** The interrupts can be configured first in Ct_Ppg_Init(), after initialization, 
 ** a PPG interrupt can be enabled by the function Ct_Ppg_EnableIrq().
 ** This function can set callback function for each channel too.
 **
 ** With Ct_Ppg_WriteLowWidthVal() the PPG low width is set to the value
 ** given in the parameter Ct_Ppg_WriteLowWidthVal#u16Cycle.
 **
 ** With Ct_Ppg_WriteHighWidthVal() the PPG high width is set to the value
 ** given in the parameter Ct_Ppg_WriteHighWidthVal#u16Cycle.
 **
 ** Notes that PPG can be only set to 16-bit mode, so above two parameters
 ** should be 16-bit.
 **
 ** Ct_Ppg_EnableCount() is used to enabe PPG counter.
 **
 ** After above setting is done, calling Ct_Ppg_EnableSwTrig() will start
 ** PPG timer.
 **
 ** With Ct_Ppg_ReadCurCnt() the current PPG count can be read when PPG timer
 ** is running.
 **
 ** With interrupt mode, when the interrupt occurs, the interrupt flag will
 ** be cleared and run into user interrupt callback function.
 **
 ** With polling mode, user can use Ct_Ppg_GetIrqFlag() to check if the
 ** interrupt occurs, and clear the interrupt flag by Ct_Ppg_ClrIrqFlag().
 **
 ** When stopping the PPG timer, use Ct_Ppg_DisableCount() to disable PPG
 ** counter and Ct_Ppg_DisableIrq() to disable PPG interrupt.
 **
 ** <b>How to use Reload Timer (RT) function of CT module?</b>
 **
 ** Ct_ConfigIOMode() must be used to configure Ct I/O mode to I/O mode 0 first.
 **
 ** Ct_Rt_Init() is used to initialize RT source clock, output polarity,
 ** operation mode and so on. Following operation mode can set:
 ** - Reload mode
 ** - One-shot mode
 **
 ** The interrupts can be configured first in Ct_Rt_Init(), after initialization, 
 ** a RT interrupt can be enabled by the function Ct_Rt_EnableIrq().
 ** This function can set callback function for each channel too.
 **
 ** RT can be set to 16-bit mode or 32-bit mode. In 16-bit mode,
 ** Ct_Rt_WriteCycleVal() set the RT counter cycle of according channel.
 ** In the 32-bit mode, Ct_Rt_WriteCycleVal() with even channel register
 ** pointer as paremater set the low 16-bit cycle value and
 ** Ct_Rt_WriteCycleVal() with odd channel register pointer as paremater
 ** set the high 16-bit cycle value.
 **
 ** Ct_Rt_EnableCount() is used to enabe RT counter.
 **
 ** After above setting is done, calling Ct_Rt_EnableSwTrig() will start
 ** Reload Timer.
 **
 ** With Ct_Rt_ReadCurCnt() the current RT count can be read when Reload Timer
 ** is running.
 **
 ** With interrupt mode, when the interrupt occurs, the interrupt flag will
 ** be cleared and run into user interrupt callback function.
 **
 ** With polling mode, user can use Ct_Rt_GetIrqFlag() to check if the
 ** interrupt occurs, and clear the interrupt flag by Rt_Ppg_ClrIrqFlag().
 **
 ** When stopping the PPG timer, use Ct_Rt_DisableCount() to disable RT
 ** counter and Ct_Rt_DisableIrq() to disable RT interrupt.
 **
 ** <b>How to use PWC timer function of CT module?</b>
 **
 ** Ct_ConfigIOMode() must be used to configure Ct I/O mode to I/O mode 0 first.
 **
 ** Ct_Pwc_Init() is used to initialize PWC timer source clock, measurement mode
 ** and so on.
 **
 ...
 ** - One-shot mode
 **
 ** The interrupts can be configured first in Ct_Pwc_Init(), after initialization, 
 ** a PWC interrupt can be enabled by the function Ct_Pwc_EnableIrq().
 ** This function can set callback function for each channel too.
 **
 ** Ct_Pwc_EnableCount() is used to enabe PWC counter.
 **
 ...
 ** with Ct_Pwc_Get16BitMeasureData() the measured value can be read after
 ** measurement completes, in the 32-bit mode, with Ct_Pwc_Get32BitMeasureData()
 ** the measured value can be read.
 **
 ** With interrupt mode, when the interrupt occurs, the interrupt flag will
 ** be cleared and the code runs into user interrupt callback function.
 **
 ** With polling mode, user can use Ct_Pwc_GetIrqFlag() to check if the
 ** completion interrupt occurs, and clear the interrupt flag by
 ** Ct_Pwc_ClrIrqFlag(). Ct_Pwc_GetErrorFlag() is used to get the error flag
 ** of PWC measurement.
 **
 ** When stopping the PWC timer, use Ct_Pwc_DisableCount() to disable PWC
 ** counter and Ct_Pwc_DisableIrq() to disable PWC interrupt.
 **
 ******************************************************************************/
...
typedef enum en_ct_channel
{
    ...
    CtChannel4     = 4u,  ///< channel 4
    CtChannel5     = 5u,  ///< channel 5
    CtChannel6     = 6u,  ///< channel 6
    CtChannel7     = 7u,  ///< channel 7
    CtChannelMax   = 8u,  ///< channel max
} en_ct_channel_t;
...
#define CT4       (*((volatile stc_ctn_t *) M0P_CT4_PWM_BASE))
#define CT5       (*((volatile stc_ctn_t *) M0P_CT5_PWM_BASE))
#define CT6       (*((volatile stc_ctn_t *) M0P_CT6_PWM_BASE))
#define CT7       (*((volatile stc_ctn_t *) M0P_CT7_PWM_BASE))
